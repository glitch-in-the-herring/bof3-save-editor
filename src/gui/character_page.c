...
{
    char buffer[16];
    static unsigned long uint8_signal_ids[7];
    ...
    other_signals[0] = g_signal_connect(character_data_fields->character_fields->name_entry, "changed", G_CALLBACK(store_name_character_entry), slot_page_ids[17]);
    ...
    other_signals[1] = g_signal_connect(character_data_fields->character_fields->exp_entry, "changed", G_CALLBACK(store_exp_character_entry), slot_page_ids[18]);
    ...
    for (int i = 0; i < 10; i++)
    {
        slot_page_ids[i + 7]->entry_id = i + 7;        
