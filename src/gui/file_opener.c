...
void file_opener_open(GtkWidget *widget, gpointer data)
{
    ...
    {
        loadable->file = gtk_file_chooser_get_file(GTK_FILE_CHOOSER(open_dialog));
        ...
        if (g_file_load_contents(loadable->file, NULL, (char **) &memory_card, NULL, NULL, NULL))
        {
            if (validate_memory_card(memory_card))
            {
                ...
                if (loadable->file != NULL)
                    g_object_unref(loadable->file);

                for (int i = 0; i < 3; i++)
                {
                    for (int j = 0; j < 8; j++)
                    {
                        if (slot_page->save_slots[i]->character_data != NULL)
                            g_free(slot_page->save_slots[i]->character_data[j]);
                    }
                }     
                ...
                loadable->card_stream->file_stream = g_file_open_readwrite(G_FILE(loadable->file), NULL, NULL);
            }
            else
            {
        ...
        {
            if ((filename = g_file_get_path(loadable->file)) != NULL)
            {
